...
local image_file = {
    ...
    { name = 'Кастет', file = getWorkingDirectory() .. "\\Mordor Shop Interfaces\\Images\\kastet.png" },
    { name = 'Гольф-клюшка', file = getWorkingDirectory() .. "\\Mordor Shop Interfaces\\Images\\golf.png" },
    { name = 'Лопата', file = getWorkingDirectory() .. "\\Mordor Shop Interfaces\\Images\\shovel.png" },
    { name = 'Бильярдный кий', file = getWorkingDirectory() .. "\\Mordor Shop Interfaces\\Images\\kiy.png" },
    { name = 'Катана', file = getWorkingDirectory() .. "\\Mordor Shop Interfaces\\Images\\katana.png" },
    { name = 'Пустынный орел', file = getWorkingDirectory() .. "\\Mordor Shop Interfaces\\Images\\deagle.png" },
    { name = 'AK-47', file = getWorkingDirectory() .. "\\Mordor Shop Interfaces\\Images\\ak47.png" },
    { name = 'M4A1', file = getWorkingDirectory() .. "\\Mordor Shop Interfaces\\Images\\m4a1.png" },
    { name = 'Винтовка', file = getWorkingDirectory() .. "\\Mordor Shop Interfaces\\Images\\awm.png" },
    { name = 'Бронежилет', file = getWorkingDirectory() .. "\\Mordor Shop Interfaces\\Images\\armor.png" },
    { name = 'Талон', file = getWorkingDirectory() .. "\\Mordor Shop Interfaces\\Images\\talon.png" },
    { name = 'Патрон .50', file = getWorkingDirectory() .. "\\Mordor Shop Interfaces\\Images\\pt50.png" },
    { name = 'Патрон 5.56', file = getWorkingDirectory() .. "\\Mordor Shop Interfaces\\Images\\pt556.png" },
    { name = 'Патрон 7.62', file = getWorkingDirectory() .. "\\Mordor Shop Interfaces\\Images\\pt762.png" }
}
...
local update_url = 'https://github.com/deffix-def/Mordor-Shop-Inferfaces/raw/main/update_msi.ini'
local update_path = getWorkingDirectory() .. "\\update_msi.ini"
local script_url = 'https://github.com/deffix-def/Mordor-Shop-Inferfaces/raw/refs/heads/main/Mordor%20Shop%20Interfaces.lua'
    ...
    downloadUrlToFile(update_url, update_path, function(id, status)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            ...
            if not updateIni then
                sampAddChatMessage(tag .. 'Нет доступных обновлений!', -1)
                return
            end
            if updateIni.info.vers > script_vers then
                sampAddChatMessage(tag .. "Доступна {32CD32}новая {FFFFFF}версия скрипта: {32CD32}"..tostring(updateIni.info.vers).."{FFFFFF}.", -1)
                sampAddChatMessage(tag .. "Введите {32CD32}/update_msi {FFFFFF}для обновления.", -1)
                sampRegisterChatCommand('update_msi', function() update_state = true end)
            else
                sampAddChatMessage(tag .. 'Нет доступных обновлений!', -1)
            end
...
function imgui.CustomMenuItem(index, item, size, duration)
    ...
    local size = size or imgui.ImVec2(135, 25)
    local duration = duration or 0.22
    ...
    pool.pos1 = bringVec2To(
        (pool.hovered or nav.sel[0] == index) and imgui.ImVec2(p.x + 10, p.y + size.y / 2) or imgui.ImVec2(p.x + 10, p.y),
        (pool.hovered or nav.sel[0] == index) and imgui.ImVec2(p.x + 10, p.y) or imgui.ImVec2(p.x + 10, p.y + size.y / 2),
        pool.h_start,
        duration
    )
    pool.pos2 = bringVec2To(
        (pool.hovered or nav.sel[0] == index) and imgui.ImVec2(p.x + size.x + 10, p.y + size.y / 2) or imgui.ImVec2(p.x + size.x + 10, p.y + size.y),
        (pool.hovered or nav.sel[0] == index) and imgui.ImVec2(p.x + size.x + 10, p.y + size.y) or imgui.ImVec2(p.x + size.x + 10, p.y + size.y / 2),
        pool.h_start,
        duration
    )
    pool.pos3 = bringVec2To(
        nav.sel[0] == index and imgui.ImVec2(p.x + 10 + size.x / 2, p.y + size.y - 3) or imgui.ImVec2(p.x + 10, p.y + size.y - 3),
        nav.sel[0] == index and imgui.ImVec2(p.x + 10, p.y + size.y - 3) or imgui.ImVec2(p.x + 10 + size.x / 2, p.y + size.y - 3),
        pool.a_start,
        duration
    )
    pool.pos4 = bringVec2To(
        nav.sel[0] == index and imgui.ImVec2(p.x + 10 + size.x / 2, p.y + size.y - 1) or imgui.ImVec2(p.x + 10 + size.x, p.y + size.y - 1),
        nav.sel[0] == index and imgui.ImVec2(p.x + 10 + size.x, p.y + size.y - 1) or imgui.ImVec2(p.x + 10 + size.x / 2, p.y + size.y - 1),
        pool.a_start,
        duration
    )
    ...
    DL:AddRectFilled(pool.pos1, pool.pos2, imgui.GetColorU32Vec4(color1), 5, 15)
    DL:AddRectFilled(pool.pos3, pool.pos4, imgui.GetColorU32Vec4(color2), 5, 30)
    local st = imgui.CalcTextSize(item.name)
    imgui.PushFont(ammo_font)
    if item.name == 'Холодное оружие' then
        imgui.SameLine(20)
    else
        imgui.SameLine(170   + (index - 2) * 134 + (134 - imgui.CalcTextSize(u8(item.name)).x) / 2)
    end
...
function main()
    ...
    sampRegisterChatCommand('check_update_msi', function() check_update() end)
    while true do
        wait(0)
        if update_state then
            downloadUrlToFile(script_url, script_path, function(id, status)
                if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                    sampAddChatMessage(tag .. "{FFFFFF}Скрипт {32CD32}успешно {FFFFFF}обновлён.", -1)
                end
            end)
            break
        end
...
imgui.OnFrame(function() 
    return shop.alpha > 0.00 and (not isGamePaused() and not isPauseMenuActive())
        ...
        imgui.BeginChild('shop_items', imgui.ImVec2(845, 560), false)
            if shop_type:find('Оружейная') then
                imgui.SetCursorPosY(imgui.GetCursorPosY() + 5)
                for i, item in ipairs(nav.list) do
                    local size = item.name == 'Холодное оружие' and imgui.ImVec2(155, 26) or imgui.ImVec2(124, 26)
                    if imgui.CustomMenuItem(i, item, size) then
                        buy_settings.ammo_clicked_i = i - 1
                        sampSendChat('/use')
                    end
                    if i ~= #nav.list then
                        imgui.SameLine(165 + (i - 1) * 134)
                    end
                end
            end
...
imgui.OnFrame(function()
    return shop_buy.alpha > 0.00 and (not isGamePaused() and not isPauseMenuActive())
        ...
        if shop_type == 'Заправочная станция' and (title:find('АИ') or title:find('Газ') or title:find('Дизель')) then
            ...
            size_check(title)
            imgui.SetCursorPosX((130 - imgui.CalcTextSize(u8(tostring(new_num(price) .. '$'))).x) / 2 + 20)
        ...
        if shop_type == 'Заправочная станция' and (title:find('АИ') or title:find('Газ') or title:find('ДТ')) then
            ...
            if shop_type:find('Оружейная') and title:find('Патрон') then
                imgui.Text(u8'Количество приобретаемого товара')
                imgui.SameLine()
                imgui.TextDisabled(u8'(1 товар = 10 пт)')
            else
                imgui.Text(u8'Количество приобретаемого товара')
            end
            imgui.SetNextItemWidth(212)
            ...
            if not first then
                imgui.SetCursorPosX(170)
                imgui.Text(u8'Вы заплатите: ' .. new_num(tostring(buy_settings.count[0] * tonumber(price))) .. '$')
            end
        end
        imgui.End()
        imgui.PopStyleVar()
...
function sampev.onShowDialog(id, style, title, button1, button2, text)
    ...
    if not buy_settings.buy_bool then
        if id == 4431 and (title:find('Холодное оружие') or title:find('Пистолеты') or title:find('Автоматы') or title:find('Винтовки') or title:find('Боеприпасы') or title:find('Прочее')) then
            item_list = {}
            check_items = true
            if check_items then
        ...
        if (id == 4427 and (title:find('24/7') or title:find('Бар') or title:find('Стрип%-клуб') or title:find('Закусочная Burger Shot') or title:find('Закусочная Rusty Brown') or title:find('Закусочная Cluckin Bell') or title:find('Закусочная Pizzeria'))) or (id == 32700 and (title:find('Заправочная станция') or title:find('Тип топлива'))) or (id == 4430 and title:find('Оружейная')) then
            if not shop.state then
                ...
                check_items = true
                if title:find('Оружейная') then
                    if buy_settings.ammo_clicked_i > -1 or buy_settings.ammo_leave then
                        check_items = false
                    end
                end
                shop.switch()
        ...
        if id == 4430 and title:find('Оружейная') and text:find('Холодное оружие') then
            if check_items then
                sampSendDialogResponse(4430, 0, _, _)
                check_items = false
                return false
            else
                if buy_settings.ammo_leave then
                    sampSendDialogResponse(4430, 0, _, _)
                    buy_settings.ammo_leave = false
                    return false
                else
                    sampSendDialogResponse(4430, 1, buy_settings.ammo_clicked_i, _)
                    buy_settings.last_ammo_clicked_i = buy_settings.ammo_clicked_i
                    buy_settings.ammo_clicked_i = -1
                    return false
                end
            end
        end
        if id == 4431 and (title:find('Холодное оружие') or title:find('Пистолеты') or title:find('Автоматы') or title:find('Винтовки') or title:find('Боеприпасы') or title:find('Прочее')) then
            if check_items then
                sampSendDialogResponse(4431, 0, _, _)
                check_items = false
                buy_settings.ammo_leave = true
                return false
            end
        end
        ...
        if not buy_settings.find then
            ...
            elseif (id == 4431 and (title:find('Холодное оружие') or title:find('Пистолеты') or title:find('Автоматы') or title:find('Винтовки') or title:find('Боеприпасы') or title:find('Прочее'))) or (id == 4430 and title:find('Оружейная')) then
                if id == 4430 and title:find('Оружейная') and text:find('Холодное оружие') then
                    sampSendDialogResponse(4430, 1, buy_settings.last_ammo_clicked_i, _)
                    return false
                end
                if id == 4431 and (title:find('Холодное оружие') or title:find('Пистолеты') or title:find('Автоматы') or title:find('Винтовки') or title:find('Боеприпасы') or title:find('Прочее')) then
                    buy_settings.find = true
                    buy_settings.buy_bool = false
                    sampSendDialogResponse(4431, 1, buy_settings.i, 0)
                    return false
                end
            end
        end
...
function item_line_check(line)
    if shop_type:find('Оружейная') then
        if line:match("Холодное оружие") or line:match("Пистолеты") or line:match("Автоматы") or line:match("Винтовки") or line:match("Боеприпасы") or line:match("Прочее") then
            local found_name = false    
            for i, item in ipairs(nav.list) do
                if item.name == line then
                    found_name = true
                    break
                end
            end
            if not found_name then
                table.insert(nav.list, { name=line })
            end
        end
    end
    if line:match(".+%d+.+") then
        ...
        local title, price
        if line:find('Патрон') then
            title, price = line:match("^(Патрон .+)%s+(%d+)")
        else
            title, price = line:match("^(.-)%s+(%d+)")
        end
...
imgui.OnInitialize(function()
    ...
    ammo_font = imgui.GetIO().Fonts:AddFontFromFileTTF(getFolderPath(0x14) .. '\\trebucbd.ttf', 17.5, _, glyph_ranges)
