...
local shop = { state = true, duration = 0.15 }
local shop_buy = { state = true, duration = 0.3 }
...
function sampev.onShowDialog(id, style, title, button1, button2, text)
    ...
    if not buy_settings.buy_bool then
        if id == 4427 and title:find('24/7') then
            if not shop.state then
                shop_type = 'Магазин 24/7'
                check_items = true
                shop.switch()
            end
            if check_items then
...
imgui.OnInitialize(function()
    ...
    for i, image in ipairs(image_file) do
        if imgui_textures[image.name] == nil then
